...
function HealthWellbeing(props) {
    ...
    useEffect(() => {
        let hw = props.networkParser.calculateHealthWellbeing(props.climatePrediction, props.year);
    ...
    }, [props.networkParser, props.climatePrediction, props.year]);
    ...
    return (
        <LoadingOverlay active={props.loading} spinner text={"Loading climate data"}>
                    ...
                    healthNodes.map((node) => (
                        <div key={node.node_id} className={"vert-container-health"}>
                            ...
                            <div className="health-text">
                                {node.label} {node.state == "increase" ? "increasing" : "decreasing"}
                            </div>
                        </div>
                    ))
                ) : (
            ...
            <p className="note">
                Data source: The impact data is based on published scientific literature and reports.{" "}
                <a
                ...
                , and the references relevant to particular impacts can be explored in the health impact details
                network.
            </p>
        </LoadingOverlay>
    );
