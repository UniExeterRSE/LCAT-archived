...
function NetworkListener(props) {
    ...
    }, [props.networkParser, props.year, props.climatePrediction, props.sector]);
    return null;
}
        ...
        select: async function (event) {
            if (event.nodes.length > 0) {
                ...
                if (node.state.value == "uncertain") {
                    info.explanation =
                        "No direction (positive or negative) is displayed for " +
                        node.label +
                        ", as the impacts that affect it are mixed.";
                    dir = "mixed impact";
                }
        ...
        for (let node of props.networkParser.nodes) {
            ...
            if (liveNode != null) {
                ...
                updateNodesList.push({
                    id: liveNode.id,
                    image: await networkRenderer.nodeImageURL(node, false, grey),
                });
                let col = "#115158";
                if (grey) {
            ...
            <div {...getCollapseProps()}>
                <div className="content">
                    ...
                    <p>
                        ...
                        averages. You can explore the network by clicking/tapping on the nodes and connections for more
                        information. Nodes can be moved around by dragging them, and the network can also be zoomed and
                        panned. View impacts relevant to&nbsp;
                    ...
                    <p>
                        The map is exploring heat impacts on health. It is currently at prototype stage and therefore
                        incomplete.
                    </p>
                    ...
                    <NetworkListener
                        ...
                        callback={(network) => {
                            setGraph(
                                networkRenderer.buildGraph(props.networkParser, network.nodes, network.edges, sector),
                            );
                    ...
                    <FullScreen handle={handle}>
                        <div className="network">
                            ...
                            <div ref={infoRef} className="network-info">
                                ...
                                >
                                    {handle.active ? <span>Exit fullscreen</span> : <span>Show fullscreen</span>}
                                </button>
                                <h2>{info.title}</h2>
                                <p>{info.explanation}</p>
                                <p>{info.text}</p>
                                <References id={nodeedgeId} api_call={apiCall} />
