...
<circle
             style="fill:` +
            col +
            `;fill-opacity:1;stroke-width:0.46499997"
             id="circle1093-0-8"
             cx="150"
             cy="400"
             r="100" />
 <text x="50%" y="66%" text-anchor="middle" fill="white" font-size="40px" 
font-family="Arial" dy=".3em">` +
            text +
            `</text>
...
`
    ...
    async nodeImageURL(node, glow, transparent) {
        ...
        if (glow) {
            draw.group()
                .svg(await this.loadImage("glow"))
                .move(-3, icon_pos - 10);
        }

        // draw the icon
        draw.group()
            .svg(await this.loadImage("icons/" + node.label))
            .move(10, icon_pos);

        if (node.state.value != "deactivated" && node.state.value != "unknown") {
            // draw the direction
            draw.group()
                .svg(await this.loadImage(node.state.value))
                .move(54, 60);
        }
    ...
    buildGraph(networkParser, nodes, edges, sector) {
        ...
        for (let node of networkParser.nodes) {
            if (["Pressure", "Effect", "State", "Exposure"].includes(node.type) && node.state.value != "deactivated") {
